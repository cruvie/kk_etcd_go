...
func (SerEtcd) InitEtcd(stage *kk_stage.Stage, endpoints []string, userName string, password string) error {
	...
	if config.Config.RootPassword != "" {
		...
		if err != nil {
			slog.Error("etcd client init failed", kk_log.NewLog(stage.TraceId).Error(err).Args()...)
			return err
		}
	} else {
		//add root user
		...
		kk_etcd_client.EtcdClient, err = clientv3.New(cfg)
		if err != nil {
			slog.Error("etcd client create failed", kk_log.NewLog(stage.TraceId).Error(err).Args()...)
			return err
		}
			...
			if err.Error() == "etcdserver: user name not found" {
				if _, err := kk_etcd_client.EtcdClient.UserAdd(context.Background(), kk_etcd_const.UserRoot, kk_etcd_const.UserRoot); err != nil {
					if err.Error() != "etcdserver: user name already exists" {
						slog.Error("add etcd user failed", kk_log.NewLog(stage.TraceId).Error(err).Args()...)
						return err
					}
				}
			} else {
				slog.Error("get etcd user failed", kk_log.NewLog(stage.TraceId).Error(err).Args()...)
				return err
			}
		}
		...
		if _, err := kk_etcd_client.EtcdClient.RoleAdd(context.Background(), kk_etcd_const.RoleRoot); err != nil {
			if err.Error() != "etcdserver: role name already exists" {
				slog.Error("add etcd role failed", kk_log.NewLog(stage.TraceId).Error(err).Args()...)
				return err
			}
		}
		...
		if _, err := kk_etcd_client.EtcdClient.UserGrantRole(context.Background(), kk_etcd_const.UserRoot, kk_etcd_const.RoleRoot); err != nil {
			slog.Error("grant role to user failed", kk_log.NewLog(stage.TraceId).Error(err).Args()...)
			return err
		}
		//enable etcd auth
		if _, err := kk_etcd_client.EtcdClient.AuthEnable(context.Background()); err != nil {
			slog.Error("Enable Auth failed", kk_log.NewLog(stage.TraceId).Error(err).Args()...)
			return err
		}
		...
		if err != nil {
			if errors.Is(err, kk_etcd_error.KeyAlreadyExists) {
				...
				if err != nil {
					slog.Error("update admin user failed", kk_log.NewLog(stage.TraceId).Error(err).Args()...)
					return err
				}
			} else {
				slog.Error("add admin user as an administrator of the system failed", kk_log.NewLog(stage.TraceId).Args()...)
				return err
			}
		}
		...
		if err != nil {
			...
			slog.Error(errStr, kk_log.NewLog(stage.TraceId).Args()...)
			return err
		}
	}
	return nil
