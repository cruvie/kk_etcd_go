...
func (c *ClientHub[T]) GetClient(stage *kk_stage.Stage) *T {
	c.rwLock.RLock()
	if len(c.clients) == 0 {
		slog.Error("no grpc client available, attempt to proactively obtain service", kk_log.NewLog(stage.TraceId).Any("serviceType", c.serviceType).Any("serviceName", c.serviceName).Args()...)
		err, serverList := hServer.ServerList(stage, &kk_etcd_models.ServerListParam{Prefix: c.serviceType + "/" + c.serviceName})
		if err != nil {
			slog.Error("ServerList failed", kk_log.NewLog(stage.TraceId).Any("serviceType", c.serviceType).Any("serviceName", c.serviceName).Error(err).Args()...)
			return nil
		} else {
			c.rwLock.RUnlock()
			...
			c.rwLock.RLock()
			if len(c.clients) == 0 {
				slog.Error("still no grpc client available", kk_log.NewLog(stage.TraceId).Any("serviceType", c.serviceType).Any("serviceName", c.serviceName).Error(err).Args()...)
				return nil
			}
		}
	}
...
func (c *ClientHub[T]) ListenServerChange(ctx context.Context, stage *kk_stage.Stage) error {
	...
	slog.Info("start watch server list", kk_log.NewLog(stage.TraceId).Any("serviceType", c.serviceType).Any("serviceName", c.serviceName).Args()...)
	serverListChan := make(chan *kk_etcd_models.PBListServer)
	err := WatchServerList(ctx, c.serviceType+"/"+c.serviceName, serverListChan)
	if err != nil {
		slog.Error("WatchServerList failed", kk_log.NewLog(stage.TraceId).Any("serviceType", c.serviceType).Any("serviceName", c.serviceName).Error(err).Args()...)
		return err
	}
	...
	go func() {
		for {
			...
			select {
			case <-ctx.Done():
				slog.Info("ctx done stop ListenServerChange, close serverListChan", kk_log.NewLog(stage.TraceId).Any("serviceType", c.serviceType).Any("serviceName", c.serviceName).Args()...)
...
func (c *ClientHub[T]) refreshClients(stage *kk_stage.Stage, serverList *kk_etcd_models.PBListServer) {
	...
	for _, server := range serverList.ListServer {
		...
		if err != nil {
			slog.Error("grpc client connect failed", kk_log.NewLog(stage.TraceId).Error(err).Any("target", server.ServiceAddr).Args()...)
			return
		}
